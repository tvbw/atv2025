...
const extractVideos = jsonData => {
    ...
    return jsonData.Items.map(it => {
        ...
        return {
            vod_id: it.Id,
            vod_name: it.Name || "",
            vod_pic: `${host}/emby/Items/${it.Id}/Images/Primary?maxWidth=400&tag=${it.ImageTags.Primary}&quality=90`,
...
const category = async (tid, pg, _, extend) => {
    ...
    const url = `${host}/emby/Users/${Users}/Items?SortBy=DateLastContentAdded%2CSortName&SortOrder=Descending&Recursive=true&Fields=BasicSyncInfo%2CCanDelete%2CContainer%2CPrimaryImageAspectRatio%2CProductionYear&StartIndex=${startIndex}&ParentId=${tid}&EnableImageTypes=Primary%2CBackdrop%2CThumb&ImageTypeLimit=1&Limit=50&X-Emby-Token=${Token}`;
    const resp = await req(url, { headers });
    ...
    const pagecount = pg * 50 < total ? +pg + 1 : +pg;
    return JSON.stringify({ list, page: +pg, pagecount, limit: 50, total });
};
